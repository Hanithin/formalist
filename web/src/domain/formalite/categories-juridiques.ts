/**
 * Les catégories juridiques de l'INSEE, et la forme qu'elles désignent.
 *
 * Le registre ne rend pas « SARL » mais un code à quatre chiffres - 5499 - tiré de la
 * nomenclature des catégories juridiques. La table qui les traduisait tenait dix
 * entrées écrites de mémoire, et cinq étaient fausses : 5410 valait « SA » alors que
 * c'est une SARL nationale, 6532, 6533 et 6534 valaient « SCI » alors que ce sont un
 * groupement agricole, un GAEC et un groupement foncier, et deux codes - 5720, 5498 -
 * n'existent pas. Une société pouvait donc porter dans ses actes une forme qui n'est
 * pas la sienne, jusqu'au greffe.
 *
 * Elle est désormais reprise de la source, en entier : les 260 libellés officiels, et
 * le rattachement de chacun à la forme que Formalist emploie dans ses actes.
 *
 * Source : INSEE, nomenclature des catégories juridiques, niveau III, septembre 2022.
 * https://www.insee.fr/fr/statistiques/fichier/2028129/cj_septembre_2022.xls
 * Consultée le 27 août 2026. L'INSEE la révise rarement ; à la prochaine révision, il
 * faut reprendre le fichier et régénérer les deux tables ci-dessous.
 *
 * Un point à connaître : **la nomenclature ne distingue pas l'unipersonnel**. Une SASU
 * est immatriculée 5710 comme une SAS, une EURL 5499 comme une SARL. L'unipersonnalité
 * n'est pas une catégorie juridique : elle se lit au nombre d'associés, non au
 * registre. C'est pourquoi la table rend la forme pluripersonnelle, à charge pour
 * l'écran de la préciser.
 */

/** Le libellé officiel de chaque catégorie, pour pouvoir nommer ce qu'on a lu. */
...
/**
 * La forme employée dans les actes, pour les catégories qui en désignent une.
 *
 * Toutes n'en désignent pas : une association, une administration, un GIE, une société
 * étrangère n'ont pas de forme au sens où nos actes l'entendent. Leur code reste connu
 * - son libellé figure ci-dessus - mais sans rattachement, et l'écran le dit plutôt que
 * de laisser un champ vide sans explication.
 */
export const FORME_PAR_CATEGORIE: Record<string, string> = {
  "5202": "SNC",
  "5203": "SNC",
  "5306": "SCS",
  "5307": "SCS",
...
/**
 * La forme que le registre désigne, ou `null`.
 *
 * @param categorie le code à quatre chiffres rendu par l'annuaire des entreprises
 */
export function formeDeLaCategorie(categorie: string | null | undefined): string | null {
  return FORME_PAR_CATEGORIE[(categorie ?? "").trim()] ?? null;
}

/** Ce que le registre dit de la société, en toutes lettres. */
export function libelleDeLaCategorie(categorie: string | null | undefined): string | null {
  return LIBELLES_CATEGORIES[(categorie ?? "").trim()] ?? null;
}
