/**
 * pdf.cjs - file d'attente et cache de conversion PDF, par LibreOffice.
 *
 * Déplacé depuis lib/pdf.js sans être réécrit. Seuls les chemins changent : Next
 * réécrit __dirname en /ROOT/ dans le paquet produit, et le dossier temporaire
 * s'y retrouvait inaccessible sur toute machine sans /dev/shm - macOS compris.
 * L'ancien lib/pdf.js réexporte ce fichier.
 */
...
/**
 * Dossier de travail des conversions.
 *
 * /dev/shm est un disque en mémoire : la conversion y est nettement plus rapide.
 * Il n'existe que sous Linux, d'où le repli - qui passe par le dossier temporaire
 * du système plutôt que par __dirname, réécrit par l'empaqueteur.
 */
const TMP = fs.existsSync("/dev/shm")
  ? "/dev/shm/formalist"
  : path.join(os.tmpdir(), "formalist-conversions");
