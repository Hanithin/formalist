...
import { actionsAttendues, prochaineEtape } from "@/domain/formalite/actions";
...
export async function tableauDeBord(utilisateur: UtilisateurConnecte) {
  ...
  const [contextes, nonLus, derniers, journal] = await Promise.all([
    /*
     * Ce que chaque dossier attend.
     *
     * Le contexte s'assemblait ici, avec ses deux requêtes de comptage. Il est parti
     * dans un constructeur commun le jour où la liste des formalités en a eu besoin :
     * deux copies de « comment on décide ce qu'un dossier attend » auraient dérivé.
     */
    contextesDesDossiers(dossiers),
